            <%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ListarSolicitudTransfusionExterna.aspx.cs" Inherits="ComprobantesRetencion.ListarSolicitudTransfusionExterna" %>
...
<head>
    ...
    <script type = "text/javascript">
        ...
        function fnAtender(Id) {
                location.href = '../transfusion/AtenderSolicitudTransfusionExterna.aspx?cod='+Id;
        }
            ...
            if (document.getElementById("txtfini").value !="" && document.getElementById("txtffin").value != "" && document.getElementById("cmbEstado").value !="0") {
                $.ajax({
                    ...
                    success: function (data) {
                        debugger;
                        document.getElementById('tbTransfusionesBody').innerHTML = '<tbody id="tbTransfusionesBody"></tbody>';
                        $.each(data.d, function (key, val) {

                            var row = $("<tr />");
                            $("<td />").text(val.NroSolicitud).appendTo(row);
                            $("<td />").text(val.Paciente).appendTo(row);
                            $("<td />").text(val.DesEstado).appendTo(row);
                            $("<td />").text(val.FechaRegistro).appendTo(row);
                            $("<td />").text(val.FechaModificacion).appendTo(row);
                            $("<div class='btn-group btn-group-sm'><button type='button' class='btn btn-oval btn-success' onclick='fnAtender(" + val.idSolicitud + ")'> Atender</button></div>").appendTo(row);

                            $(row).appendTo("#tbTransfusionesBody");
                        })
                    },
                ...
                <div class="header-block header-block-search hidden-sm-down">
                    <form role="search">
                        <div class="input-container">
                            <h3 class="title">Lista de Solicitud de Transfusión Externa </h3>
                        </div>
                    </form>
                </div>
            ...
            <aside class="sidebar">
             ...
             <article class="content forms-page">
                <div class="subtitle-block">
                  <h3 class="subtitle"> Lista de Solicitud de Transfusión Externa </h3>
               </div>
